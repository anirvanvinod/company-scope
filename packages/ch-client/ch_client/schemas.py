...
class CHDocumentMetadata(_CHBase):
    """
    Response for GET /document/{document_id} from the CH Document API.

    The Document API is a separate service from the Public Data API and uses
    a different base URL.  It returns metadata about a cached filing document,
    including the content types that can be downloaded.

    available_content_types is derived from the keys of the resources dict
    (the format most reliably present across CH API versions).
    """

    company_number: Optional[str] = None
    barcode: Optional[str] = None
    significant_date: Optional[date] = None
    significant_date_type: Optional[str] = None
    category: Optional[str] = None
    pages: Optional[int] = None
    content_length: Optional[int] = None
    content_type: Optional[str] = None  # default/preferred content type
    filename: Optional[str] = None
    # resources maps content-type → some resource reference
    resources: Optional[dict[str, Any]] = None
    links: Optional[dict[str, Any]] = None

    @property
    def available_content_types(self) -> list[str]:
        """Return downloadable content types derived from the resources dict."""
        if self.resources:
